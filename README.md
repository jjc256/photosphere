...
| `js/ba.js` | Geometry back-end: SO(3) exp/log, homography RANSAC, **focal length from the homographies** (OpenCV `focalsFromHomography`), a two-term **radial lens model** (`x_d = x_u(1 + k₁r_u² + k₂r_u⁴)`) with both directions, per-pair relative-rotation refinement (L2 Levenberg–Marquardt + IRLS outlier re-filter), **global L2 rotation averaging** anchored to the IMU, and **gain compensation**. |
