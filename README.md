...
| `solver/` + `js/ba.js` | Rust WebAssembly generalized-camera kernel (Gennery projection) plus geometry back-end: SO(3) exp/log, homography RANSAC, **focal length from the homographies**, a calibrated generalized projection (`L`) and two-term radial model (`k₁`, `k₂`), per-pair refinement, global rotation averaging, bundle adjustment, and gain compensation. |
